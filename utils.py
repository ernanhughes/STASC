...
class ThresholdOptimizerClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        for threshold in np.linspace(min_val, max_val, num=200):
...
def add_ideal(ds, without_context_col, with_context_col):
    ...
    final_results_with_ci = unc_based_KM_with_bootstrap(
        ds,
        with_context_col=with_context_col,
        without_context_col=without_context_col,
        unc_col="ideal_unc",
    )
    ...
    general_metrics = [
        ...
        "",
        "",
        np.around(final_results_with_ci['In-Accuracy']['init_mean'], 3),

    ]


    for metric_name, all_stats in final_results_with_ci.items():
        for stat_name, stat_value in all_stats.items():
            detailed_metrics.append(np.around(stat_value, 3))
...
def unc_based_KM_with_bootstrap(ds, with_context_col, without_context_col, unc_col, num_rounds=1000):
    ...
    # Collect metric values from the dataset
    for elem in ds:
        need_retrieval = elem[unc_col]
        if need_retrieval:
            ans_col = with_context_col
        ...
        metrics["In-Accuracy"].append(elem[in_accuracy_col])
        metrics["Mean Tokens"].append(num_tokens)
        metrics["EM"].append(elem[em_col])
        metrics["F1"].append(elem[f1_col])

    # Convert metrics to DataFrame for bootstrapping
    metrics_df = pd.DataFrame(metrics)

    np.random.seed(42)

    # Initialize arrays for bootstrapped metrics
    bootstrap_results = {
        "In-Accuracy": [],
        "Mean Tokens": [],
        "EM": [],
    ...
    for i in range(num_rounds):
        sampled_df = metrics_df.sample(frac=1.0, replace=True)
        for metric_name in bootstrap_results.keys():
            bootstrap_results[metric_name].append(np.mean(sampled_df[metric_name]))
    ...
    for metric_name, values in bootstrap_results.items():
        ...
        results_with_ci[metric_name] = {
            "init_mean": np.mean(metrics[metric_name]),
            "bootstrap_mean": mean_value,
            "CI_lower": lower_bound, 
            "CI_higher": upper_bound,
            "std": np.std(values)
        }
